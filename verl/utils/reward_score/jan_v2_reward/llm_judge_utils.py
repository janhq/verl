...
def evaluate_answer(question: str, solution: str, golden_label: str) -> dict[str, str]:
    predicted_answer_raw = solution
    gold_answer = golden_label
    ...
    if not question or not gold_answer:
        # Cannot grade if essential info is missing
        grade_letter = "C" 
        ...
        if llm_raw_response == ERROR_NO_API_KEY:
            ...
            print(
                f"Warning: LLM returned unexpected/empty content. Marking as ERROR.",
                file=sys.stderr,
            )
        else:
            ...
            if match:
                ...
                grade_description = CHOICE_LETTER_TO_STRING.get(
                    grade_letter, "UNKNOWN_GRADE")
            else:
                grade_letter = "ERROR"
                grade_description = "LLM Did Not Return A/B/C"
                print(
                    f"Warning: LLM returned unexpected response format: '{llm_raw_response}'. Marking as ERROR.",
                    file=sys.stderr,
                )
